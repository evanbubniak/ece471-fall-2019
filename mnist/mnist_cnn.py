    ...
    def build_dataset(self, dataset_kind):
        ...
        for label in self.labels:
            ...
            for img_file in os.listdir(img_path):
                img = self.get_nparray_from_imgfile(os.path.join(img_path, img_file))
                X.append(img)
                y.append(label)
...
# Data downloaded from https://www.kaggle.com/jidhumohan/mnist-png/downloads/mnist-png.zip as a zip of pngs
...
class Model:
    def __init__(self):
        ...
        self.sequential_model = Sequential([
            Conv2D(data.X_dim, (3, 3), activation='relu', input_shape = (data.X_dim, data.X_dim, 1)),
            MaxPooling2D(pool_size=(2, 2)),
            Dropout(DROPOUT_RATIO),
            Flatten(),
            Dense(data.num_labels, activation='softmax', kernel_regularizer=keras.regularizers.l2(L2_PENALTY))
            ])
